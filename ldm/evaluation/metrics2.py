    ...
    def calc_metrics(self, samples, targets, masks=None):
        ...
        if masks is not None:
            ...
            for i in range(bs): #apply cell mask
                ...
                #Old code
                mses_for_edist[i] = torch.mul(mses[i], masks[i].repeat(3, 1, 1)) #still need for edists
                #maes[i] = torch.mul(maes[i], masks[i].repeat(3, 1, 1))
                #ssims[i] = torch.mul(ssims[i], masks[i][5:251, 5:251].repeat(3, 1, 1))
        ...
        return mse_per_chan, mae_per_chan, ssim_per_chan, iou_per_chan, edists_per_chan, pcc_per_chan, cos_per_chan, cell_area

def calc_dino_metrics():
    #TO DO
    return
