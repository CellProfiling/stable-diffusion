...
class ImageLogger(Callback):
    def __init__(self, batch_frequency, max_images, clamp=True, increase_log_steps=True,
                 ...
                 log_to_slack=None, log_images_kwargs=None, log_validation=False, val_batch_frequency=1, log_test=False, test_batch_frequency=1, monitor_val_metric=None):
    ...
    def on_test_batch_end(self, trainer, pl_module, outputs, batch, batch_idx, dataloader_idx):
        print(f"Process {os.getpid()} in on_test_batch_end(), batch_idx {batch_idx}, global step {pl_module.global_step}")
        if self.log_test and batch_idx % self.test_batch_frequency == 0 and not self.disabled and pl_module.global_step > 0:
            print(f"Logging test images in batch {batch_idx}")
