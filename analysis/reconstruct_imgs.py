...
def dino_permute_chans(img):
  ...
  #DINO: Nuc (DAPI), ER, RNA, AGP, Mito
  # --> np.transpose(img, (4, 3, 2, 1, 0))
  return img[[4, 3, 2, 1, 0], :, :]
...
def main(opt):
  ...
  DINOdir = f"{savedir}/DINO_images_224"
  recondir = f"{savedir}/reconstructions"
  ...
  with torch.no_grad():
    with model.ema_scope():
      for i in range(0, len(data.datasets["test"]), batch_size): #loop through number of batches
        ...
        #GET IMAGES
        recons = torch.clip(recons, min=-1, max=1) #Is this necessary? --> yes
        ...
        #SEPARATE CHANNELS INTO REF/OUTPUT
        if "ldm" in model_name:
        ...
        for j in range(len(recons)):
          #print(info)
          if make_webapp: #Save images for visualizing in Webapp
            Image.fromarray(recons[j].astype(np.uint8), 'RGB').save(imgdir + "/fake_" + str(counter) + ".png")
            Image.fromarray(originals[j].astype(np.uint8), 'RGB').save(imgdir + "/real_" + str(counter) + ".png")
            if "ldm" in model_name:
              Image.fromarray(refs[j].astype(np.uint8), 'RGB').save(imgdir + "/ref_" + str(counter) + ".png")
            counter+=1

          if make_dino: #Save single_cell images for feeding to DINO
            dino_size = 224
            recon = recons[j]
            original = originals[j]
            ref = refs[j][:,:, 0:2]

            #change order of channels and resize to 224x224 for dino format
            original = np.concatenate([ref, original], axis=2)
            original = order_img(original, channel_order)
            original = dino_permute_chans(original)
            original = original.transpose((1,2,0))
            original = cv2.resize(original, dsize=(dino_size, dino_size), interpolation=cv2.INTER_LINEAR) #resize
            ...
            recon = cv2.resize(recon, dsize=(dino_size, dino_size), interpolation=cv2.INTER_LINEAR) #resize
            recon = recon.transpose((2,0,1))

            for i in range(recon.shape[0]):#loop through channels
              channel = recon[i,:,:].astype(np.uint8)
              imname = f"{recondir}/{image_ids[j]}p01-ch{i+1}sk1fk1fl1_{str(subtiles[j])}.png"
              os.makedirs(os.path.dirname(imname), exist_ok=True)
              imwrite(imname, channel)
          ...
          if True:
            ...
            num_labels = len(np.unique(mask)) #include 0 for background
            #print("Total numer of cells: " + str(num_labels))
            #print("bits in labeled mask: ")
            ...
            for k in range(1, num_labels):
              single_cell_mask = np.array(mask == k)
              ...
              #resizing mask (not needed after fixing dataloader)
              single_cell_mask = single_cell_mask.transpose((1,2,0))
              single_cell_mask = cv2.resize(single_cell_mask, dsize=(dino_size, dino_size), interpolation=cv2.INTER_NEAREST) #resize
              single_cell_mask = single_cell_mask.transpose((2,0,1))
              assert single_cell_mask.shape == (5, dino_size, dino_size)
              ...
              assert original_dino_format.shape == (dino_size, dino_size*6)
              assert recon_dino_format.shape == (dino_size, dino_size*6)
...
if __name__ == "__main__":
  ...
  parser.add_argument(
    "--Webapp",
    action="store_true",
    help="Save Webapp formatted images",
  )
  parser.add_argument(
    "--DINO",
    action="store_true",
    help="Save DINO formatted images",
  )
  parser.add_argument(
    "--savedir",
    type=str,
